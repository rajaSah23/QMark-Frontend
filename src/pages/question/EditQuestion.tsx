...
import { updateQuestion } from '../../store/action/question-action';
...
const EditQuestion = ({ setIsModelOpen, questionData }: any) => {
    const dispatch = useDispatch<AppDispatch>();
    const [isSaving, setIsSaving] = useState(false);
    const [opened, { open, close }] = useDisclosure(false);
    
    // Determine the index of the correct answer from the options array
    ...
    const form = useForm({
        initialValues: {
            ...
            correctAnswer: correctAnswerIndex !== -1 ? correctAnswerIndex : -1,
            explanation: questionData?.explanation || '',
            difficulty: questionData?.difficulty || 'easy',
            subject: questionData?.subject?._id || questionData?.subject || "",
            topic: questionData?.topic?._id || questionData?.topic || "",
    ...
    const removeOption = (idx: number) => {
        if (form.values.options.length > 2) {
            ...
            } else if (form.values.correctAnswer > idx) {
                form.setFieldValue('correctAnswer', form.values.correctAnswer - 1);
            }
        }
    };
    ...
    const handleQuestionSubmit = (data: any) => {
        ...
        const submitData = {
            ...
            correctAnswer: data.options[data.correctAnswer]
        };
        
        setIsSaving(true);
        dispatch(updateQuestion(submitData))
            .then(() => {
                setIsModelOpen(false);
            }).catch(() => {
                toast.error("Question not updated", "Please try later");
            })
            .finally(() => {
                setIsSaving(false);
            });
