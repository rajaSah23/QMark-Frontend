...
export const addSubject = createAsyncThunk("addSubject", async (payload: any, { rejectWithValue }) => {
    try {
        // Using the standalone subject endpoint (no topics required)
        const response = await apiClient.POST("/master/subject", payload);
...
export const addSubjectWithTopics = createAsyncThunk("addSubjectWithTopics", async (payload: any, { rejectWithValue }) => {
    try {
